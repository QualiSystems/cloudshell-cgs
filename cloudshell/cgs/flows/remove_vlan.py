...
from cloudshell.cgs.command_actions.vlan import VlanActions
from cloudshell.cgs.errors import FilterDoesNotExist
...
class CgsRemoveVlanFlow(RemoveVlanFlow):
    ...
    def execute_flow(
        ...
        with self._cli_handler.get_cli_service(
            ...
            if port_mode == "trunk":
                filter_ids = vlan_actions.get_trunk_vlan_filters_id(
                    port=port,
                ...
                filter_ids = vlan_actions.get_access_vlan_filters_id(
                    port=port,
                    vlan=vlan_range,
                    action_map=action_map,
                    error_map=error_map,
            ...
            if not filter_ids:
                raise FilterDoesNotExist(
                    "Unable to find filter for port {} VLAN {}".format(port, vlan_range)
                )
