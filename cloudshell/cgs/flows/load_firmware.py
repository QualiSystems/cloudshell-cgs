...
from cloudshell.cgs.command_actions.firmware import FirmwareActions
...
class CgsLoadFirmwareFlow(LoadFirmwareFlow):
    ...
    def execute_flow(self, path, vrf, timeout):
        ...
        with self._cli_handler.get_cli_service(
            self._cli_handler.sw_upgrade_config_mode
        ) as config_session:
            commit_actions = CommitActions(
                cli_service=config_session, logger=self._logger
            )
            firmware_actions = FirmwareActions(
                cli_service=config_session, logger=self._logger
            )

            try:
                firmware_actions.set_remote_firmware_file(remote_url=path)
                firmware_actions.set_remote_firmware_user(
                    user=url.get(UrlParser.USERNAME)
                )
                firmware_actions.set_remote_firmware_password(
            ...
            boot_bank = firmware_actions.get_sw_upgrade_file_boot_bank(
                file_name=file_name
            )
    ...
    def _wait_for_image_validation(self, firmware_actions, file_name):
        ...
        timeout_time = datetime.now() + timedelta(
            seconds=self.IMAGE_VALIDATION_WAITING_TIMEOUT
        )
        status = ""
        ...
        while status.lower() != self.IMAGE_VALIDATION_SUCCESS_STATE.lower():
            ...
            if error_msg:
                raise Exception(
                    "Unable to validate firmware file. {}".format(error_msg)
                )
