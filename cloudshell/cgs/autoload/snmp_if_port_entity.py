from cloudshell.cgs.autoload.snmp_if_entity import SnmpIfEntity
...
class SnmpIfPortEntity(SnmpIfEntity):
    ...
    def __init__(
        ...
        super(SnmpIfPortEntity, self).__init__(
    ...
    def if_speed(self):
        if not self._if_speed:
            self._if_speed = self._snmp.get_property(
                self.IF_MIB, self.IF_SPEED, self.if_index, "int"
            )
    ...
    def if_mtu(self):
        if not self._if_mtu:
            self._if_mtu = self._snmp.get_property(
                self.IF_MIB, self.IF_MTU, self.if_index, "int"
            )
        return self._if_mtu
