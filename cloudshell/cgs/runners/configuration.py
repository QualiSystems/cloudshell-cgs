...
from cloudshell.cgs.flows.restore import CgsRestoreFlow
from cloudshell.cgs.flows.save import CgsSaveFlow
...
class CgsConfigurationRunner(ConfigurationRunner):
    ...
    def restore(
        ...
        :param path: relative path to the file tftp://server/sourcefile
        :param configuration_type: configuration type to restore (StartUp or Running)
        ...
        if hasattr(self.resource_config, "vrf_management_name"):
            vrf_management_name = (
                vrf_management_name or self.resource_config.vrf_management_name
            )

        self._validate_configuration_type(configuration_type)
        # todo: check with the backup type "File System"!
        #   It will fail because it passes path without adding
