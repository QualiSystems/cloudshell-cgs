...
from cloudshell.cgs.command_templates import filters
...
class FiltersActions(object):
    ...
    def get_filters(self, action_map=None, error_map=None):
        ...
        :rtype: Filters
        """
        connections = CommandTemplateExecutor(
    ...
    def remove_filters(self, filter_ids, action_map=None, error_map=None):
        ...
        if len(filter_ids) > 1:
            return CommandTemplateExecutor(
                ...
                command_template=filters.REMOVE_FILTERS,
            ).execute_command(filter_ids=",".join(filter_ids))

        elif len(filter_ids) == 1:
            return self.remove_filter(
                filter_id=filter_ids[0], action_map=action_map, error_map=error_map
            )
